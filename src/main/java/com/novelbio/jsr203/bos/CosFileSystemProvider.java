...
public class CosFileSystemProvider extends FileSystemProvider {

	private static final Logger logger = LoggerFactory.getLogger(CosFileSystemProvider.class);
	
	public static final String SCHEME = "oss";
	
	  // Checks that the given file is a HadoopPath
	static final CosPath toCosPath(Path path) {
		if (path == null) {
	...
	public FileSystem newFileSystem(URI uri, Map<String, ?> env) throws IOException {
		return new CosFileSystem(this, uri);
	}
	...
	public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
		return new ObjectSeekableByteStream(PathDetailOs.getBucket(), ((CosPath)path).getInternalPath());
	}
	...
	public InputStream newInputStream(Path path, OpenOption... options) throws IOException {
		return toCosPath(path).newInputStream(path, options);
	}
	...
	public DirectoryStream<Path> newDirectoryStream(Path dir, Filter<? super Path> filter) throws IOException {
		return toCosPath(dir).newDirectoryStream(filter);
	}

	@Override
	public void createDirectory(Path dir, FileAttribute<?>... attrs) throws IOException {
		toCosPath(dir).createDirectory(attrs);
	}

	@Override
	public void delete(Path path) throws IOException {
		toCosPath(path).delete();
	}
	...
	public void copy(Path source, Path target, CopyOption... options) throws IOException {
		toCosPath(source).copy(toCosPath(target), options);
	}

	@Override
	public void move(Path source, Path target, CopyOption... options) throws IOException {
		toCosPath(source).move(toCosPath(target), options);
	}

	@Override
	public boolean isSameFile(Path path, Path path2) throws IOException {
		return toCosPath(path).compareTo(toCosPath(path2)) == 0;
	}
...
//		throw new UnsupportedOperationException("method no supported! method=checkAccess");
		toCosPath(path).checkAccess(modes);
	}
...
//		throw new UnsupportedOperationException("method no supported! method=getFileAttributeView");
		return (V) new CosFileAttrbuteView(toCosPath(path));
	}
...
//		throw new UnsupportedOperationException("method no supported! method=readAttributes");
		// move 方法会调用
		return (A) toCosPath(path).readAttributes(type, options);
	}
